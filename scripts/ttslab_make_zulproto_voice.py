...
MAIN_MORPHDICT = "main_morphdict.pickle"
MAIN_MORPHPARSER = "main_morphparser.pickle"

def make_voice(langs, synthfile="frontend"):
    pronun = {}
    for i, lang in enumerate(langs):
        if i == 0:
            exec("from ttslab.lang.%(lang)s import NewNTTS2Voice as Voice" % {"lang": lang})
            langpref = "main"
        else:
            langpref = lang
        pronun[langpref] = {}
    ...
    if synthfile == "frontend":
        ...
        voice.morphdict = ttslab.fromfile(MAIN_MORPHDICT)
        voice.morphparser = ttslab.fromfile(MAIN_MORPHPARSER)
